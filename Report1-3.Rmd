...
cleaned_data <- orig_data %>%
  ...
  filter(str_detect(host_location, "Australia"),
         ...
         has_availability != "",
         minimum_nights <= 365,
         across(.cols = where(is.numeric) & !any_of(c("longitude", "latitude")), .fns  = ~ is.na(.x) | .x >= 0),
         price > 0,
...
cleaned_data <- cleaned_data %>%
  mutate(
    room_type = case_when(room_type == "Entire home/apt" ~ "Entire home/apt",
  ...
  select(-c("neighbourhood_cleansed","latitude", "longitude", "bathrooms_text", "host_location","host_since", "first_review", "last_scraped","license","has_availability"))
```
