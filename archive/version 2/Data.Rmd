...
age_levels <- c("15-19 years","20-24 years","25-29 years","30-34 years","35-39 years",
...
combined <- combined %>%
  mutate(AgeGroup_ord = factor(AgeGroup,levels = age_levels, ordered = TRUE),
         Marital = replace_na(Marital, "Unknown"),
         EmploymentType = ifelse(
      `employment_Employed full-time` > `employment_Employed part-time`,
      "Full-time", "Part-time"
    ) %>% factor(levels = c("Part-time", "Full-time")),
         YearMonth_date = as.Date(as.yearmon(YearMonth, format = "%m-%Y")),
         Year = year(YearMonth_date),
         Month = month(YearMonth_date)) %>%
  select(-c('AgeGroup','Employment to population ratio', 'population_total')) %>%
  filter(!is.na(AgeGroup_ord)) %>%
    select(
    YearMonth_date, Year, Month,
    Sex, Marital, AgeGroup_ord,
    `Civilian population aged 15 years and over`,
    EmploymentType
  )

table(combined$EmploymentType)
```



```{r}
write.csv(combined, "dataset.csv", row.names = FALSE)
```

To what extent can demographic and temporal characteristics (sex, marital status, age group, civilian population and time) be used to classify whether employed individuals work full-time or part-time?
...
plot_categorical_distribution(combined)
```

```{r}
# for modelling part
# the distribution of the sample is un balance
library(tidymodels)
library(themis)
rec_down <- recipe(EmploymentType ~ ., data = combined) %>%
  step_dummy(all_nominal_predictors()) %>%
  step_downsample(EmploymentType) 
```