...
cleaned_data <- cleaned_data %>%
  ...
  mutate(commercial_score = (availability_365 >= 300) + (minimum_nights <= 10) + (host_total_listings_count >= 5) + (instant_bookable == "1") + (room_type == "Entire home/apt"),
         commercial_score = case_when(
      commercial_score %in% c(0, 1) ~ 1,
      commercial_score == 2         ~ 2,
      commercial_score == 3         ~ 3,
      commercial_score %in% c(4, 5) ~ 4),
...
Leakage control: The variables used to define the proxy outcome (`availability_365`, `minimum_nights`, `host_total_listings_count`, `instant_bookable`) are not included as predictors. This separation reduces label leakage risk and helps explainability.
